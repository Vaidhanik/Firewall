...
class NetworkInterceptor:
    ...
    # def _create_linux_rule(self, app_path: str, target_ip: str, action: str = 'add') -> bool:
    #     """Create iptables rule for Linux systems"""
    #     try:
    #         # First try to get absolute path of the application
    #         try:
    #             app_path = subprocess.check_output(['which', app_path]).decode().strip()
    #         except subprocess.CalledProcessError:
    #             # If 'which' fails, use the provided path
    #             pass
    ...
    #         if action == 'add':
    #             # First add rule for established connections
    #             established_cmd = [
    #                 'sudo', 'iptables',
    #                 '-A', 'OUTPUT',
    #                 '-p', 'tcp',
    ...
    #                 '-j', 'DROP'
    #             ]
    #             subprocess.run(established_cmd, check=True)

    #             # Then add rule for UDP traffic
    #             udp_cmd = [
    #                 'sudo', 'iptables',
    #                 '-A', 'OUTPUT',
    #                 '-p', 'udp',
    ...
    #                 '-j', 'DROP'
    #             ]
    #             subprocess.run(udp_cmd, check=True)

    #         else:  # Remove rules
    #             # Find and remove rules with our comment
    #             # List all rules
    #             rules = subprocess.check_output(['sudo', 'iptables', '-L', 'OUTPUT', '--line-numbers', '-n']).decode()
    ...
    #             # Remove rules in reverse order (to maintain rule numbers)
    ...
    #         self.logger.info(f"{'Added' if action == 'add' else 'Removed'} iptables rules for {app_path} -> {target_ip}")
    #         return True

    #     except subprocess.CalledProcessError as e:
    #         self.logger.error(f"Failed to {'add' if action == 'add' else 'remove'} iptables rules: {e}")
    #         return False
